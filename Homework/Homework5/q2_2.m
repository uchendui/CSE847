...
for i=1:size(num_comps,1)
    ...
    for j=1:2
        ...
        imshow(image');
        truesize([300 200]);
        title(['Reconstructed Image ' num2str(j) ' for ' num2str(k) ' Components'])
        er = norm(image - reshape(X_c(j,:), 16, 16));
        ['Reconstruction Error for image ' num2str(j) ' with ' num2str(k) ' Components: ' num2str(er)]
    end
